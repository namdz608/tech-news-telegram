...
/**
 * Hằng `defaultAiTopics` lưu cấu hình hoặc helper dùng trong module.
 *
 * Được sử dụng tại:
 * - Không có caller trực tiếp; đây là helper nội bộ hoặc entry contract.
 */
...
export class GitHubReposCrawler implements NewsCrawler<GitHubReposSourceConfig> {
  ...
  async crawl(source: GitHubReposSourceConfig): Promise<Article[]> {
    ...
    try {
      /**
       * Hàm `repositories` thực hiện bước xử lý được mô tả bởi tên và kiểu trả về.
       *
       * Được sử dụng tại:
       * - Không có caller trực tiếp; đây là helper nội bộ hoặc entry contract.
       */
  ...
  private async fetchRepositories(source: GitHubReposSourceConfig): Promise<GitHubRepository[]> {
    /**
     * Hàm `queries` thực hiện bước xử lý được mô tả bởi tên và kiểu trả về.
     *
     * Được sử dụng tại:
     * - Không có caller trực tiếp; đây là helper nội bộ hoặc entry contract.
     */
    ...
    /**
     * Hàm `repositoryGroups` thực hiện bước xử lý được mô tả bởi tên và kiểu trả về.
     *
     * Được sử dụng tại:
     * - Không có caller trực tiếp; đây là helper nội bộ hoặc entry contract.
     */
...
function buildQueries(source: GitHubReposSourceConfig): string[] {
  /**
   * Hàm `customQuery` thực hiện bước xử lý được mô tả bởi tên và kiểu trả về.
   *
   * Được sử dụng tại:
   * - Không có caller trực tiếp; đây là helper nội bộ hoặc entry contract.
   */
...
function buildDefaultQueries(lookbackDays: number): string[] {
  /**
   * Hàm `fromDate` thực hiện bước xử lý được mô tả bởi tên và kiểu trả về.
   *
   * Được sử dụng tại:
   * - Không có caller trực tiếp; đây là helper nội bộ hoặc entry contract.
   */
...
function dedupeRepositories(repositories: GitHubRepository[]): GitHubRepository[] {
  /**
   * Hàm `seenUrls` thực hiện bước xử lý được mô tả bởi tên và kiểu trả về.
   *
   * Được sử dụng tại:
   * - Không có caller trực tiếp; đây là helper nội bộ hoặc entry contract.
   */
